#CS4391.001 Sammy Elhindi 11.14.24
"""
...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    ### ADD YOUR CODE HERE ###
    # Must use the weight_coord and weight_noobj defined above
        # Coordinate losses (x, y, w, h)
    loss_x = weight_coord * torch.sum(box_mask * torch.pow(
        gt_box[:, 0:1] - output[:, 0:5*num_boxes:5], 2.0))
    loss_y = weight_coord * torch.sum(box_mask * torch.pow(
        gt_box[:, 1:2] - output[:, 1:5*num_boxes:5], 2.0))
    

    # Width and height losses using square root as per YOLO paper
    loss_w = weight_coord * torch.sum(box_mask * torch.pow(
        torch.sqrt(gt_box[:, 2:3]) - torch.sqrt(output[:, 2:5*num_boxes:5]), 2.0))
    
    loss_h = weight_coord * torch.sum(box_mask * torch.pow(
        torch.sqrt(gt_box[:, 3:4]) - torch.sqrt(output[:, 3:5*num_boxes:5]), 2.0))
    
    # Object confidence loss (for cells with objects)
    loss_obj = torch.sum(box_mask * torch.pow(
        box_confidence - output[:, 4:5*num_boxes:5], 2.0))
    
    # No-object confidence loss (for cells without objects)
    # For each grid cell, we penalize ALL boxes that weren't selected as the responsible box
    
    noobj_mask = 1 - box_mask  # Inverse of box_mask
    loss_noobj = weight_noobj * torch.sum(
        noobj_mask * torch.pow(output[:, 4:5*num_boxes:5], 2.0))
    
    # Class prediction loss (only for cells containing objects)
    
    loss_cls = torch.sum(gt_mask.unsqueeze(1) * torch.pow(
        gt_box[:, 4:5] - output[:, 5*num_boxes:], 2.0))
