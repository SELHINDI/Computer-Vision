...
if __name__ == '__main__':
    # Check if CUDA is available. Ignore, I wanted to use mps but gave up.
    ...
    network.to(device)  # Move the model to the appropriate device
    ...
    for epoch in range(num_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            image = sample['image'].to(device)  # Move the image to the device
            gt_box = sample['gt_box'].to(device)  # Move the ground truth box to the device
            gt_mask = sample['gt_mask'].to(device)  # Move the ground truth mask to the device
