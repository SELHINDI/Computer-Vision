...
from layers import fc_forward, fc_backward, l2_loss, relu_backward, relu_forward
...
def training_step(model, X_batch, y_batch, reg):
    ...
    ###########################################################################
    ###########################################################################
  # Initialize loss and gradient dictionary
    loss, grads = 0.0, {}

    # frwrd pass through the two-layer network
    scores, cache = model.forward(X_batch)

    # compure the softmax (cross-entropy) loss and the gradient of the loss w.r.t. scores
    data_loss, grad_scores = softmax_loss(scores, y_batch)
#add L2 regularization to the loss for each weight matrix
    reg_loss_W1, grad_W1_reg = l2_regularization(model.W1, reg)
    reg_loss_W2, grad_W2_reg = l2_regularization(model.W2, reg)
    reg_loss = reg_loss_W1 + reg_loss_W2
    loss = data_loss + reg_loss


    # Backward pass to compute gradients for W1, b1, W2, and b2
    grads = model.backward(grad_scores, cache)
    # Add L2 regularization gradients for W1 and W2
    grads['W1'] += grad_W1_reg
    grads['W2'] += grad_W2_reg
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return loss, grads
